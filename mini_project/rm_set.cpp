...
void Rm_set::set_target_rpm(int id, int rpm){
    ...
    this->target_rpm[id] = this->target_rpm[id] > this->MAX_OUTPUT ? this->MAX_OUTPUT : (this->target_rpm[id] < -this->MAX_OUTPUT ? -this->MAX_OUTPUT : this->target_rpm[id]);
}
...
void Rm_set::run_rpm_pid_cycle(int id){
    ...
    for (int m = 0; m < 4; m++){
      this->can_msg[m * 2] = (unsigned char)(this->output[m] >> 8);
      this->can_msg[m * 2 + 1] = this->output[m] & 0xff;
    }
}
...
void Rm_set::update_motor_status(int id, long micros, int pos, int rpm){
    ...
    */
    ...
    if (this->motor_mode[id] == 1){  // position control
      ...
      // ganchulia principle
      this->target_rpm[id] = dp > 20000 ? 5000 : (dp < -20000 ? -5000 : ( dp > 1000 ? 1000 : (dp < -1000 ? -1000 : ( dp > 100 ? 200 : (dp < -100 ? -200 : 0)))));
    }
