...
class TakeOff_Class:
    ...
    def test_takeoff(self):
        ...
        StartingPosition = self.drone.get_state(HomeChanged)
        print("StartingPosition:", StartingPosition)
        self.drone(FlyingStateChanged(TakeOff(_no_expect=True)& FlyingStateChanged(state="hovering", _timeout=10, _policy="check_wait"))).wait()
        print("%%%%%%%%% self.Drone finished taking off %%%%%%%%%%%%%%%%%%%%%")
        #Get self.drone position 
        TakeOffLocation = self.drone.get_state(PositionChanged)
        ...
        time.sleep(3)
        # Get self.drone position 
        WayPoint1_Location = self.drone.get_state(PositionChanged)
        ...
        time.sleep(4)
        # Get self.drone position
        self.drone_location = self.drone.get_state(PositionChanged)
        '''
    ...
    def connection(self):
        self.drone = olympe.Drone(self.ANAFI_IP)
        self.drone.connect()
